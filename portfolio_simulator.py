...
# ------------------------------------------------------------
# CAPNOW — Multi-Portfolio Simulation Lab
# • One main studio to create deals, jump in calendar, renew/default per deal
# • Calendar engine: Mon–Fri collections
# • Waterfall: 60/40 until investors hit 60% ROI, then 25/75; +1.5% fee on PROFITS
# • Default investors preloaded; editable
...
PRE_HURDLE_SPLIT_INV = 0.60  # 60/40 before 60% ROI
PRE_HURDLE_SPLIT_CAP = 0.40
POST_HURDLE_SPLIT_INV = 0.25 # 25/75 above the hurdle
POST_HURDLE_SPLIT_CAP = 0.75
MGMT_FEE = 0.015             # 1.5% of PROFITS (not ending cash)
MIN_TICKET = 5_000.0
...
# ------------------------------------------------------------
# SIDEBAR — Create/Open portfolio + Investors & Launch
# ------------------------------------------------------------
...
with se2:
    st.metric("End date", p.end_date.strftime("%Y-%m-%d"))

studio_tab, perf_tab, finalize_tab = st.tabs(["🧪 Studio", "📈 Performance", "📌 Finalize & Compare"])
...
with perf_tab:
    ...
    if p.deals:
        ...
        st.info("No deals yet. Create them in the **Studio** tab.")
...
with finalize_tab:
    ...
    st.markdown("---")
    st.markdown("### Portfolio Run Snapshots")
    if st.button("📌 Save snapshot to comparison", use_container_width=True):
        ...
        st.success("Saved! See the table below.")
